...
def test_grin_reflection_detailed():
    ...
    ax2.axvline(
        x=turning_z, color="green", linestyle="--", alpha=0.5, label="Turning point"
    )
    ...
    ax4 = axes[1, 1]
    distance = np.cumsum(
        np.sqrt(np.diff(np.array(z_points)) ** 2 + np.diff(np.array(x_points)) ** 2)
    )
