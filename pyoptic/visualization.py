...
class LensViewer:
    ...
    def view(self, fields='all', wavelengths='primary', num_rays=3, distribution='line_y',
             figsize=(10, 4)):
    ...
    def _plot_all_surfaces(self, ax):
        ...
        for k in range(1, self.optic.surface_group.num_surfaces-1):
            if n[k] > 1:
                ...
                y1 = np.linspace(-self._real_ray_extent[k], self._real_ray_extent[k], 128)
                z1 = surf1.geometry.sag(y=y1) + surf1.geometry.cs.z

                y2 = np.linspace(-self._real_ray_extent[k+1], self._real_ray_extent[k+1], 128)
                ...
                elif n[k+1] != n[k] and n[k+1] != 1:  # bonded lens, first lens
                    y3 = np.linspace(-self._real_ray_extent[k+2], self._real_ray_extent[k+2], 128)
                    min_radius = min(np.min(y1), np.min(y2), np.min(y3))
                    max_radius = max(np.max(y1), np.max(y2), np.max(y3))

                elif n[k+1] != n[k] and n[k-1] != n[k]:  # bonded lens, second lens
                    y0 = np.linspace(-self._real_ray_extent[k-1], self._real_ray_extent[k-1], 128)
                ...
                polygon = Polygon(vertices, closed=True, facecolor='lightgray', edgecolor='gray')
                ax.add_patch(polygon)

        # plot image surface
        yi = np.linspace(-self._real_ray_extent[-1], self._real_ray_extent[-1], 128)
        image_surf = self.optic.image_surface
        zi = image_surf.geometry.sag(y=yi) + image_surf.geometry.cs.z
        ax.plot(zi, yi, 'gray')

    def _plot_rays(self, ax, fields='all', wavelengths='primary', num_rays=3, distribution='line_y'):
        if fields == 'all':
            ...
            for wavelength in wavelengths:
                ...
                for k in range(y.shape[0]):
                    if np.max(np.abs(y[k, :])) > self._real_ray_extent[k]:
                        ...
                        self._real_ray_extent[k] = max([np.max(np.abs(y[k, :])), surf.semi_aperture])
