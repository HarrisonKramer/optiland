...
import os
from PySide6.QtCore import Qt, Slot, QPropertyAnimation, QEasingCurve, QTimer, QRect # Added QRect
...
THEME_LIGHT_PATH = os.path.join(os.path.dirname(__file__), "resources", "light_theme.qss")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.lensEditorDock.toggleViewAction().triggered.connect(
            lambda checked, dock=self.lensEditorDock: self.animate_dock_toggle(dock, checked)
        )
        ...
        self.systemPropertiesDock.toggleViewAction().triggered.connect(
            lambda checked, dock=self.systemPropertiesDock: self.animate_dock_toggle(dock, checked)
        )
        ...
        self.analysisPanelDock.toggleViewAction().triggered.connect(
            lambda checked, dock=self.analysisPanelDock: self.animate_dock_toggle(dock, checked)
        )
        ...
        self.optimizationPanelDock.toggleViewAction().triggered.connect(
            lambda checked, dock=self.optimizationPanelDock: self.animate_dock_toggle(dock, checked)
        )
    ...
    def animate_dock_toggle(self, dock_widget, show_state_after_toggle):
        ...
        is_currently_visible = not dock_widget.isHidden() # More reliable for animation start
        ...
        original_width = self.dock_original_sizes.get(dock_widget, 300) # Default if not found
        ...
        if show_state_after_toggle: # Animate IN
            if dock_widget.isHidden(): # Only animate if truly hidden
                ...
                if self.lensEditorDock.isTabbed() and dock_widget in [self.lensEditorDock, self.systemPropertiesDock]:
                    parent_tab_bar = self.lensEditorDock.parentWidget().parentWidget() # Rough guess, might need QTabBar specific parent
                ...
                if self.dockWidgetArea(dock_widget) in [Qt.DockWidgetArea.LeftDockWidgetArea, Qt.DockWidgetArea.RightDockWidgetArea]:
                    ...
                    animation.setStartValue(0)
                    animation.setEndValue(dock_widget.sizeHint().height() if dock_widget.sizeHint().height() > 0 else 200)
            ...
            if not dock_widget.isHidden(): # Only animate if truly visible
                ...
                animation.finished.connect(lambda: dock_widget.setMaximumWidth(original_width) if self.dockWidgetArea(dock_widget) in [Qt.DockWidgetArea.LeftDockWidgetArea, Qt.DockWidgetArea.RightDockWidgetArea] else dock_widget.setMaximumHeight(2000)) # A large value
                animation.start(QPropertyAnimation.DeleteWhenStopped)
                self.dock_animations[dock_widget] = animation
