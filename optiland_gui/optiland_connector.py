...
class SpecialFloatEncoder(json.JSONEncoder):
    def default(self, obj):
        ...
        if be.isinf(obj) and obj > 0:
            return "Infinity"  # Check if be.isinf is the function
        if be.isinf(obj) and obj < 0:
            return "-Infinity"
...
class OptilandConnector(QObject):
    ...
    def _ensure_basic_optic_structure(self, optic_instance: Optic):
        ...
        if optic_instance.wavelengths.num_wavelengths == 0:
            print(
                f"Connector: Optic has no wavelengths. Adding default "
                f"primary: {self.DEFAULT_WAVELENGTH_UM} um."
            )
            ...
            print(
                "Connector: Optic has wavelengths but no primary. Setting "
                "first as primary."
            )
            if optic_instance.wavelengths.num_wavelengths > 0:
    ...
    def _get_safe_primary_wavelength_value(self) -> float:
        ...
        if self._optic.wavelengths.num_wavelengths > 0:
            ...
            print(
                "Warning: Primary wavelength index issue. Attempting to use "
                "first wavelength."
            )
            self._optic.wavelengths.wavelengths[0].is_primary = True
            for i in range(
                1, self._optic.wavelengths.num_wavelengths
            ):  # Ensure only one primary
                self._optic.wavelengths.wavelengths[i].is_primary = False
            return self._optic.wavelengths.wavelengths[0].value

        # This case means the wavelength list is empty, which
        # _ensure_basic_optic_structure should prevent.
        print(
            f"Critical Warning: No wavelengths in optic. Falling back to "
            f"default {self.DEFAULT_WAVELENGTH_UM} um."
        )
    ...
    def load_optic_from_file(self, filepath):
        try:
            ...
            self._ensure_basic_optic_structure(
                self._optic
            )  # Ensure basics even after load
    ...
    def get_surface_data(self, row, col_idx):
        ...
        if col_idx == self.COL_TYPE:
            ...
            )  # Material of space *after* this surface
            if (
                row == self.get_surface_count() - 1 and row > 0
            ):  # Image surface (not object also)
                relevant_material = self._optic.surface_group.surfaces[
                    row - 1
                ].material_post  # Material before image
            ...
            # self._optic.update_paraxial() # Might be too heavy here, ensure it's
        ...
        try:
            ...
            if col_idx == self.COL_COMMENT:
                ...
                if new_material_name.lower() == "mirror":
                    ...
                    # Material post should be same as pre for mirrors to avoid issues if
                ...
                try:
                    ...
                    print(
                        f"Semi-diameter for surface {row} set to auto "
                        f"(aperture removed)."
                    )
            ...
            print(
                f"OpticConnector: Error setting data at "
                f"({row},{col_idx}) to '{value_str}': {e}"
            )
    ...
    def add_surface(self, index=-1):  # index is LDE index
        ...
        if optic_insert_idx < 1:  # Cannot insert before first optical surface (idx 1)
            optic_insert_idx = 1
        if (
            index != -1 and 0 < index < num_lde_rows - 1
        ):  # if specific intermediate row is given
        ...
        params = {
            ...
            "radius": float("inf"),
            "thickness": 5.0,
            "material": "Air",  # material is for the space *after* new surface
            "comment": "New Surface",
    ...
    def remove_surface(self, lde_row_index):
        ...
        if 0 < optic_surface_index < self.get_surface_count() - 1:
            ...
            print(
                "OpticConnector: Cannot remove Object or Image surface via this "
                "LDE action."
            )
