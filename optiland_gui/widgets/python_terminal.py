...
from PySide6.QtGui import QAction, QColor, QFont, QIcon, QSyntaxHighlighter, QTextCharFormat
...
class PythonHighlighter(QSyntaxHighlighter):
    ...
    def _build_rules(self):
        ...
        keywords = [
            ...
            "\\btry\\b", "\\bwhile\\b", "\\bwith\\b", "\\byield\\b", "\\bself\\b",
        ]
...
class PythonTerminalWidget(QWidget):
    ...
    def __init__(self, parent=None, custom_variables=None, theme="dark"):
        ...
        self.kernel_manager.kernel.shell.events.register("post_execute", self._on_kernel_execute)
    ...
    def _define_themes(self):
        self.themes = {
            "dark": {"keyword": QColor("#CF8A2E"), "string": QColor("#A2E05D"), "comment": QColor("#9E9E9E"), "number": QColor("#B5CEA8"), "editor_bg": "#2B2B2B", "editor_fg": "#F8F8F2", "jupyter_style": "monokai", "btn_hover": "#555555", "splitter": "#555555", "title_snippets":"#F8F8F2"},
            "light": {"keyword": QColor("#0000FF"), "string": QColor("#008000"), "comment": QColor("#808080"), "number": QColor("#A31515"), "editor_bg": "#FFFFFF", "editor_fg": "#000000", "jupyter_style": "default", "btn_hover": "#EEEEEE", "splitter": "#FFFFFF", "title_snippets":"#2B2B2B"}
        }
    ...
    def _create_editor_dock(self):
        ...
        for btn in [self.btn_new_script, self.btn_run_script, self.btn_save_script, self.btn_load_script, self.btn_save_quick_action]:
    ...
    def _create_new_tab(self, content="", file_path=None):
        ...
        # Set tab title
        title = os.path.basename(file_path) if file_path else f"Untitled-{self.untitled_script_counter}"
    ...
    def _refresh_snippets_list(self):
        ...
        for name in CODE_SNIPPETS: self.snippets_list.addItem(QListWidgetItem(name))
    ...
    def _show_snippets_context_menu(self, position):
        ...
        for name in CODE_SNIPPETS.keys():
            ...
            action.triggered.connect(lambda checked=False, n=name: self._insert_snippet(n))
    ...
    def _save_script(self, index_to_save=None):
        ...
        if not file_path:
            file_path, _ = QFileDialog.getSaveFileName(self, "Save Script", "", "Python Files (*.py)")
            if not file_path:
    ...
    def _load_script(self):
        ...
        editor = self._get_current_editor()
        if editor and not editor.toPlainText().strip() and not editor.property("is_modified"):
             editor.setPlainText(content)
    ...
    def _save_as_quick_action(self):
        editor = self._get_current_editor()
        if not editor or not editor.toPlainText().strip(): return
        text, ok = QInputDialog.getText(self, "Save Quick-Action", "Enter a name for this action:")
        if ok and text:
    ...
    def set_theme(self, theme_name="dark"):
        ...
        self.snippets_title_label.setStyleSheet(f"font-weight: bold; padding: 2px; color: {theme_palette['title_snippets']};")
        ...
        self.btn_save_quick_action.setIcon(QIcon(f":/icons/{theme_name}/add_quick_action.svg"))
        self.script_tabs.setStyleSheet(f"QTabBar::close-button {{ image: url(:/icons/{theme_name}/close.svg); }} QTabBar::close-button:hover {{ background: #DB0000; }}")
        
        self.jupyter_widget.syntax_style = theme_palette["jupyter_style"]
        self.jupyter_widget._style_sheet_changed()
        self.jupyter_widget.setStyleSheet(f"QWidget {{ background-color: {theme_palette['editor_bg']}; color: {theme_palette['editor_fg']}; }}")
        ...
        for i in range(self.script_tabs.count()):
            ...
            if highlighter: highlighter.update_theme(theme_palette)
    ...
    def shutdown_kernel(self):
        if self.kernel_client and self.kernel_client.channels_running: self.kernel_client.stop_channels()
        if self.kernel_manager and self.kernel_manager.is_alive(): self.kernel_manager.shutdown_kernel()