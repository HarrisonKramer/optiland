...
from PySide6.QtWidgets import QWidget, QHBoxLayout, QLabel, QMenuBar, QPushButton, QSizePolicy, QSpacerItem
...
class CustomTitleBar(QWidget):
    ...
    def __init__(self, main_menu_bar_instance: QMenuBar, parent=None):
        ...
        if self.main_menu_bar:
            ...
            self.main_menu_bar.setSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Preferred)
        ...
        self.project_label = QLabel("Current Project: UnnamedProject.opds") # Placeholder text
        self.project_label.setObjectName("TitleBarProjectLabel")
        # Allow it to expand but not excessively, vertical preferred
        self.project_label.setSizePolicy(QSizePolicy.Policy.MinimumExpanding, QSizePolicy.Policy.Preferred) 
        ...
        self.maximize_button = QPushButton("[]") # Placeholder for maximize/restore icon
        ...
        self._mouse_move_offset = None # Store offset from window top-left to click point
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            self._mouse_move_offset = event.globalPosition().toPoint() - self.window().frameGeometry().topLeft()
        super().mousePressEvent(event)

    def mouseMoveEvent(self, event):
        if self._mouse_move_offset is not None and event.buttons() == Qt.MouseButton.LeftButton:
