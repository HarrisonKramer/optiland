...
class HuygensMTF(BaseMTF):
    ...
    def __init__(
        ...
        if be.get_backend() != "numpy":
            raise ValueError(
                "HuygensMTF (and HuygensPSF) only supports the 'numpy' backend at this"
                " time."
            )
    ...
    def _get_psf_for_field(self, field_coords):
        ...
        if cache_key not in self.psf_cache:
            ...
            # HuygensPSF.psf can be None if computation fails (e.g. no rays hit
            # detector)
    ...
    def _generate_mtf_data(self):
        ...
        """
        if be.get_backend() != "numpy":
            # Redundant check if __init__ has it, but good for safety if called
            # directly.
        ...
        for field_coords in self.fields:
            ...
            if psf is None or psf_pixel_pitch is None:
                # This means PSF calculation failed (e.g. no rays, or other HuygensPSF
                # issue)
                # Return zero MTF for this field.
                calculated_mtfs.append(
                    [be.zeros_like(self.freq), be.zeros_like(self.freq)]
                )
                continue
