...
class GeometricMTF(BaseMTF):
    ...
    def _compute_single_mtf_profile(self, ray_coords, scale_factor):
        ...
        Args:
            ray_coords (be.ndarray): Array of ray intersection coordinates (e.g., x or y).
        ...
        # self.num_points is the number of points for the MTF curve (length of self.freq)
        ...
        if len(ray_coords_clean) == 0: # Handle cases where all rays were NaN/inf or filtered out
             return be.zeros_like(self.freq)
        ...
        A, edges = be.histogram(ray_coords_clean, bins=num_hist_bins, range=[min_coord, max_coord])
        ...
        mtf_profile = be.zeros_like(self.freq) # Using be.zeros_like for backend compatibility
        ...
        if sum_A_dx == 0: # Avoid division by zero if no rays in histogram or dx is zero
    ...
    def _generate_mtf_data(self):
        ...
        if self.spot_diagram_data is None:
            ...
            raise RuntimeError("Spot diagram data has not been generated. This indicates an issue in initialization.")
        ...
        for field_idx in range(len(self.fields)):
            ...
            # TODO: Implement more sophisticated Tangential/Sagittal orientation based on field angle
            ...
            mtf_t = self._compute_single_mtf_profile(coords_for_tangential, scale_factor)
