...
class BaseMTF(ABC):
    ...
    def _parse_fields(self, fields_arg):
        ...
        if fields_arg == "all":
            ...
            raise ValueError(
                "Invalid 'fields' argument. Must be 'all', or a list of tuples"
            )
    ...
    def _parse_wavelength(self, wavelength_arg):
        ...
        if wavelength_arg == "primary":
            ...
            raise ValueError(
                "Invalid 'wavelength' argument. Must be 'primary' or a numeric value."
            )
    ...
    def _parse_max_freq(self, max_freq_arg):
        ...
        if max_freq_arg == "cutoff":
            ...
            if self.FNO == 0:  # Avoid division by zero for afocal systems or error
                # Default to a reasonable max frequency if FNO is zero, e.g.
                # 100 cycles/mm
                ...
                # import warnings
                # warnings.warn("FNO is zero, 'cutoff' frequency is ill-defined.
                # Defaulting max_freq to 100 cycles/mm.")
            else:
            ...
            raise ValueError(
                "Invalid 'max_freq' argument. Must be 'cutoff' or a numeric value."
            )
    ...
    def _get_effective_FNO(self):
        ...
        if self.optic.object_surface.is_infinite:
            ...
            # fno_effective = fno_paraxial * (1 + be.abs(magnification_m) /
            # pupil_magnification_p)
            # return fno_effective
            # Re-evaluating: Optiland's paraxial.FNO() is typically the infinite
            #  conjugate FNO.
            # The correction for finite conjugates is usually
            #  FNO_working = FNO_infinite * (1 + |m|) for distant objects
            ...
            if not self.optic.object_surface.is_infinite:
                # This is the formula from BasePSF, assume it's correct for
                # effective FNO
    ...
    def _calculate_diffraction_limit(self):
        ...
        Uses the formula:
        MTF(ν) = (2/π) * [arccos(ν/ν_c) - (ν/ν_c) * sqrt(1 - (ν/ν_c)^2)]
        ...
        if self.FNO == 0:  # Afocal system, or error
            # Diffraction limit is ill-defined or MTF is 1 everywhere up to some
            # resolution limit
            # For now, return array of 1s, implies perfect resolution up to
            # self.max_freq
            # This needs careful consideration for afocal systems.
            # import warnings
            # warnings.warn("FNO is zero, diffraction limit calculation may be
            #  inaccurate.")
            return be.ones_like(self.freq)

        cutoff_freq_theoretical = 1.0 / (wavelength_mm * self.FNO)
        if (
            cutoff_freq_theoretical == 0
        ):  # Should not happen if FNO > 0 and wavelength > 0
            return be.ones_like(self.freq)

        norm_freq = self.freq / cutoff_freq_theoretical

        # Clip norm_freq to avoid issues with arccos for values slightly > 1 due to
        # precision
        ...
        # The formula naturally handles this if norm_freq can exceed 1, then arccos
        #  becomes NaN.
    ...
    def view(
        self, figsize=(10, 6), add_reference=True, title="Modulation Transfer Function"
    ):
        ...
        for i, field_data in enumerate(self.mtf_data):
            if not (isinstance(field_data, (list, tuple)) and len(field_data) == 2):
                print(
                    f"Warning: MTF data for field {self.fields[i]} is not in the "
                    f"expected format [tangential, sagittal]. Skipping."
                )
                continue
