...
from .through_focus import ThroughFocusAnalysis
# We need backend for RMS calculation if not already imported
# import optiland.backend as be # Already imported in SpotDiagram
...
class ThroughFocusSpotDiagram(ThroughFocusAnalysis):
    ...
    def __init__(
        ...
        if coordinates not in ["global", "local"]:
            raise ValueError("Coordinates must be 'global' or 'local'.")
        self.coordinates = coordinates
        # self.results is initialized and populated by super().__init__
        # which calls _calculate_through_focus, which in turn calls
        # self._perform_analysis_at_focus for each focal step.

    def _perform_analysis_at_focus(
        self, current_delta_focus: float
        ...
        """
        # Instantiate a SpotDiagram for the optic's current focal state.
        # The optic.image_surface.geometry.cs.z has already been updated
        # by the base class's _calculate_through_focus method.
    ...
    def view(self):
        ...
        """
        print(
            "Through-Focus Spot Diagram Results "
            "(RMS radius at primary wavelength):"
        )
