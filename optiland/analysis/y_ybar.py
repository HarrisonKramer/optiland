...
class YYbar:
    ...
    def __init__(self, optic, wavelength="primary"):  #
        ...
        if isinstance(wavelength, str) and wavelength.lower() == "primary":
            if (
                optic.wavelengths.primary_index is not None
                and 0
                <= optic.wavelengths.primary_index
                < optic.wavelengths.num_wavelengths
            ):
                self.wavelength_value = optic.wavelengths.wavelengths[
                    optic.wavelengths.primary_index
                ].value
                ...
                # Optionally, you might want to log a warning if primary_index was
                # None but wavelengths exist
            else:
                # This case should be rare if Optic/OptilandConnector ensures a default wavelength
                print(
                    "Warning (YYbar.__init__): Optic has no wavelengths. "
                    "Using a default 0.550 µm for title."
                )
            ...
            print(
                f"Warning (YYbar.__init__): Unexpected wavelength format '{wavelength}'. "
                "Using primary or default."
            )
    ...
    def view(self, fig_to_plot_on=None, figsize=(7, 5.5)):  #
        ...
        if not is_gui_embedding:
            current_fig = plt.figure(
                figsize=figsize
            )  # Create new figure for standalone
            ...
            ax = current_fig.add_subplot(
                111
            )  # Add a single subplot to the GUI's figure

        # Paraxial rays will be traced using the optic's current
        # primary wavelength setting
        try:
            ...
            ax.text(
                0.5,
                0.5,
                f"Error fetching paraxial rays:\n{e}",
                ha="center",
                va="center",
        ...
        # surface 0 = object, surface 1 = first optical surface, ...,
        # last surface = image
        num_surfaces_in_optic = self.optic.surface_group.num_surfaces

        for k in range(
            1, num_surfaces_in_optic
        ):  # Iterate through optical surfaces up to image plane
            ...
            # Ray data arrays ya, yb might have N points (0 to N-1) corresponding
            # to these N surfaces
            # Or N-1 segments (if they represent data *between* surfaces)
            # Assuming ya[k], yb[k] are heights AT surface k (0-indexed in
            ...
            if (
                k == 1
            ):  # First segment, from Object (surf 0) to First Optical Surface (surf 1)
                # Label refers to the surface the ray segment is *approaching* or
                # where it lands
                label = (
            ...
            if k < num_surfaces_in_optic - 1:  # For intermediate optical surfaces
                ...
                k == num_surfaces_in_optic - 1
            ):  # Last segment, ending at Image surface
                label = "Image"
            ...
            ax.plot(
                ...
                else None,  # Only label key surfaces for clarity
                markersize=8,  #
            )
