...
class IncoherentIrradiance:
    ...
    def view(self, figsize=(6,5), cmap="inferno", cross_section=None):
            ...
            if cross_section is not None:
                if isinstance(cross_section, tuple) and len(cross_section) == 2:
                    ...
                    if isinstance(axis_type_in, str) and \
                       axis_type_in.lower() in ["cross-x", "cross-y"] and \
                       isinstance(slice_idx_in, int):
                        plot_cross_section_requested = True
                        valid_cross_section_request = True
                        cs_axis_type = axis_type_in.lower()
                        cs_slice_idx = slice_idx_in
                    else:
                        print(f"[IncoherentIrradiance] Warning: Invalid cross_section_info format. Expected ('cross-x' or 'cross-y', int). Defaulting to 2D plot.")
                else:
                    print(f"[IncoherentIrradiance] Warning: Invalid cross_section_info type. Expected tuple. Defaulting to 2D plot.")
            ...
            if vmin_plot == vmax_plot : 
                ...
                if vmin_plot == vmax_plot: vmin_plot, vmax_plot = 0.0, 1.0 
            ...
            for f_idx, field_block in enumerate(self.irr_data):
                for w_idx, entry_data in enumerate(field_block): # entry_data is the tuple
                    ...
                    if self.user_initial_rays is not None:
                        ...
                        field_label_info_val = self.fields[f_idx][0] if isinstance(self.fields[f_idx], tuple) else self.fields[f_idx]
                        ...
                        title_str_base = f"Irradiance: Field {f_idx} {field_coord}, WL {w_idx} ({wavelength_val:.3f} um)"
                    ...
                    if plot_cross_section_requested and valid_cross_section_request:
                        self._plot_cross_section(
                            ...
                            cs_axis_type, cs_slice_idx,
                            figsize, title_str_base 
                        )
                    else:
                        # 2D plotting stuff
    ...
    def _plot_cross_section(self, irr_map_be, x_edges, y_edges, 
                                axis_type: str, slice_idx: int, 
                                figsize: tuple, title_prefix: str):
            ...
            if axis_type == "cross-x": # cross section along X, data varies with Y
                if not (0 <= slice_idx < irr_map_np.shape[0]):
                    print(f"[IncoherentIrradiance] Warning: X-slice index {slice_idx} is out of bounds for map shape {irr_map_np.shape}. Skipping plot.")
                    plt.close() 
                    return
                ...
                plt.xlabel("Y (mm)")
                plot_title += f" - X-Cross-section at X ≈ {x_centers[slice_idx]:.2f} mm (index {slice_idx})"
                
            elif axis_type == "cross-y": # cross section along Y, data varies with X
                if not (0 <= slice_idx < irr_map_np.shape[1]):
                    print(f"[IncoherentIrradiance] Warning: Y-slice index {slice_idx} is out of bounds for map shape {irr_map_np.shape}. Skipping plot.")
                    plt.close() 
                    return
                ...
                plt.xlabel("X (mm)")
                plot_title += f" - Y-Cross-section at Y ≈ {y_centers[slice_idx]:.2f} mm (index {slice_idx})"
            else: 
                plt.close()
                return

            plt.plot(coords_to_plot_against, data_to_plot, linestyle='-.')
            plt.ylabel("Irradiance (W/mm^2)")
