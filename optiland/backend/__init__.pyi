...
def ones_like(x: ArrayLike, fill_value: float | ndarray) -> ndarray: ...
def load(filename: str) -> ndarray: ...
def cast(x: ArrayLike) -> ndarray: ...
def copy(x: BEArray) -> BEArray: ...
def is_array_like(x: Any) -> bool: ...
def size(x: ndarray) -> bool: ...
def newaxis() -> None: ...
def shape(x: ndarray) -> bool: ...
def reshape(x: BEArray, shape: Sequence[int]) -> BEArray: ...
def stack(xs: Sequence[ArrayLike], axis: int = 0) -> ndarray: ...
def broadcast_to(x: BEArray, shape: Sequence[int]) -> BEArray: ...
def repeat(x: BEArray, repeats: int) -> BEArray: ...
def flip(x: BEArray) -> BEArray: ...
def meshgrid(*arrays: BEArray) -> tuple[BEArray, ...]: ...
def roll(
    x: BEArray, shift: int | Sequence[int], axis: int | tuple[int, ...] = ()
) -> BEArray: ...
def unsqueeze_last(x: BEArray) -> BEArray: ...
def tile(x: BEArray, dims: int | Sequence[int]) -> BEArray: ...
...
def histogram2d(
    x: BEArray, y: BEArray, bins: Sequence[BEArray]
) -> tuple[BEArray, BEArray, BEArray]: ...
def get_bilinear_weights(
    coords: BEArray, bin_edges: Sequence[BEArray]
) -> tuple[BEArray, BEArray]: ...
def copy_to(source: BEArray, destination: BEArray) -> None: ...

# Linear algebra
def matmul(a: BEArray, b: BEArray) -> BEArray: ...
def batched_chain_matmul3d(a: BEArray, b: BEArray, c: BEArray) -> BEArray: ...
def cross(a: BEArray, b: BEArray) -> BEArray: ...
def matrix_vector_multiply_and_squeeze(p: BEArray, E: BEArray) -> BEArray: ...
def to_complex(x: BEArray) -> BEArray: ...
def mult_p_E(p: BEArray, E: BEArray) -> BEArray: ...

# Interpolation
def interp(x: ArrayLike, xp: ArrayLike, fp: ArrayLike) -> ndarray: ...
def nearest_nd_interpolator(
    points: BEArray, values: BEArray, Hx: BEArray, Hy: BEArray
) -> BEArray: ...

# Polynomial operations
def polyfit(x: BEArray, y: BEArray, degree: int) -> BEArray: ...
...
def path_contains_points(vertices: BEArray, points: BEArray) -> BEArray: ...
