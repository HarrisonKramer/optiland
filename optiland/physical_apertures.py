...
class RectangularAperture(BaseAperture):
    ...
    def clip(self, rays):
        ...
        Args:
            rays (Rays): The rays to be clipped.
        """
        condition = (rays.x < self.x_min) | (rays.x > self.x_max) | \
                    (rays.y < self.y_min) | (rays.y > self.y_max)
        rays.clip(condition)
...
class PolygonAperture(BaseAperture):
    ...
    def clip(self, rays):
        ...
        Args:
            rays (Rays): The rays to be clipped.
        """
        points = np.column_stack((rays.x, rays.y))
        inside = self._path.contains_points(points)
        rays.clip(~inside)
