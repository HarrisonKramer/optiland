...
Kramer Harrison, 2025 (Refactored 2024 for backend agnosticism and normalization)
"""
...
class HuygensPSF(BasePSF):
    ...
    def __init__(self, optic, field, wavelength, num_rays=128, image_size=128, grid_size=1024):
        ...
        # _compute_psf now correctly normalizes, so it can be called directly here.
    ...
    def _get_pupil_data(self, Hx, Hy, wavelength_um, zero_phase: bool = False):
        ...
        # Use actual system's pupil amplitude distribution for both real and ideal calculation
        ...
        # A more robust solution would be for Wavefront.get_data() to provide global normals,
        ...
        # and that this center is at the origin of the coordinate system in which these points are given.
        if hasattr(data, 'pupil_nx') and data.pupil_nx is not None: # Prefer direct normals if available
            ...
            if Rp_pupil == 0: Rp_pupil = 1e-9 # avoid div by zero
            ...
            norm_lengths = be.where(norm_lengths == 0, 1.0, norm_lengths) # Avoid div by zero
        ...
        return (pupil_x_g, pupil_y_g, pupil_z_g, pupil_amplitude, pupil_phase,
                pupil_normal_x_g, pupil_normal_y_g, pupil_normal_z_g)
    ...
    def _get_image_plane_coords(self, Hx, Hy, wavelength_um):
        ...
        # 1. Trace rays to find the global centroid of ray intersections on the image surface.
        trace_rays_for_centroid = lens.trace(
            Hx=Hx, Hy=Hy, wavelength=wavelength_um, distribution="hexapolar", num_rays=12
        )
        ...
        # Convert to numpy then to backend array to ensure correct scalar-to-array conversion for RealRays
        P_centroid_global_rays = RealRays(
            ...
            L=be.array([0.0]), M=be.array([0.0]), N=be.array([1.0]), # Dummy directions
            opd=be.array([0.0]), intensity=be.array([1.0]), wavelength=wavelength_um
        )
        
        # 2. Transform this global patch center to the local coordinates of the image surface's CS.
        cs_image.localize(P_centroid_global_rays) # Modifies P_centroid_global_rays in place
        ...
        # 3. Create a 2D grid of offsets, to be applied in the local XY plane of the image surface CS
        ...
        # 4. Define (x,y) coordinates in the local CS of the image surface for sag calculation.
        ...
        img_z_local_sag_values = image_surface.geometry.sag(img_x_local_for_sag, img_y_local_for_sag)
        ...
        local_surface_points_rays = RealRays(
            x=local_points_x_flat, y=local_points_y_flat, z=local_points_z_flat_sag,
            L=be.zeros(num_total_points, dtype=be.float_dtype()), 
            M=be.zeros(num_total_points, dtype=be.float_dtype()), 
            N=be.ones(num_total_points, dtype=be.float_dtype()), # Pointing along local Z
    ...
    def _compute_diffraction_limited_peak_intensity(self):
        """Computes the on-axis peak intensity for an equivalent diffraction-limited system.
        ...
        wavelength_mm = wavelength_um * 1e-3 # Convert um to mm for optical calculations

        # Get pupil data with zero phase, for on-axis field (Hx=0, Hy=0)
        # This uses the actual system's pupil amplitude distribution.
        (pupil_x_g, pupil_y_g, pupil_z_g, pupil_amp, ideal_pupil_phase, # ideal_pupil_phase is zeros
         pupil_nx_g, pupil_ny_g, pupil_nz_g) = self._get_pupil_data(0.0, 0.0, wavelength_um, zero_phase=True)
        
        # Get image plane coordinates for on-axis field
        img_x_g_grid, img_y_g_grid, img_z_g_grid = self._get_image_plane_coords(0.0, 0.0, wavelength_um)
        
        center_idx = N_image // 2
        # Ensure scalar values for single point calculation by converting to NumPy scalar then back to backend if needed
        ...
        # complex_pupil_contribution uses actual pupil_amp and zero phase (ideal_pupil_phase)
    ...
    def _compute_psf(self):
        ...
        (pupil_x_g, pupil_y_g, pupil_z_g, pupil_amp, pupil_phase, # pupil_phase has aberrations
         pupil_nx_g, pupil_ny_g, pupil_nz_g) = self._get_pupil_data(Hx, Hy, wavelength_um, zero_phase=False)
        ...
        # Expand dims for broadcasting: pupil arrays (1, M_pupil_points), image arrays (N_image*N_image, 1)
        ...
        k_mm = 2.0 * be.pi / wavelength_mm 
        complex_pupil_field = pup_amp_exp * be.exp(1j * pup_phase_exp) # Use actual phase
        propagator = be.exp(1j * k_mm * R) / R
