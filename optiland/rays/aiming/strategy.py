...
class CachedAimingStrategy(RayAimingStrategy):
    ...
    def aim_ray(self, optic: "Optic", Hx: float, Hy: float, Px: float, Py: float, wavelength: float):
        ...
        if is_vectorized:
            ...
            size = max(len(arr) if hasattr(arr, '__len__') and len(arr) > 1 else 1 for arr in [Hx_arr, Hy_arr, Px_arr, Py_arr])
            ...
            for i in range(size):
                ...
                results.append(self._aim_ray_scalar(optic, optic_hash, hx, hy, px, py, wavelength))
    ...
    def _aim_ray_scalar(self, optic: "Optic", optic_hash: str, Hx: float, Hy: float, Px: float, Py: float, wavelength: float):
        ...
        if cache_key in self.cache:
            ...
            return RealRays(*cached_result, intensity=be.array(1.0), wavelength=be.array(wavelength))
        ...
        # The wrapped strategy should return a RealRays object where each attribute is a 1-element array
        if be.size(rays.x) > 1:
            # This indicates the wrapped strategy might have returned a vectorized result unexpectedly.
            # The current caching implementation expects to cache individual rays.
            # For simplicity, we'll cache the first ray's data. A more robust solution might be needed.
            result_to_cache = (
                be.to_numpy(rays.x)[0], be.to_numpy(rays.y)[0], be.to_numpy(rays.z)[0],
                be.to_numpy(rays.L)[0], be.to_numpy(rays.M)[0], be.to_numpy(rays.N)[0]
            )
        else:
            result_to_cache = (
                be.to_numpy(rays.x).item(), be.to_numpy(rays.y).item(), be.to_numpy(rays.z).item(),
                be.to_numpy(rays.L).item(), be.to_numpy(rays.M).item(), be.to_numpy(rays.N).item()
            )
