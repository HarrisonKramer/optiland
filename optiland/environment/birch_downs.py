...
# α1 = −2.2106 × 10−8 Pa−1 (this is likely an erratum in my notes, should be C^-1 Pa^-1 or K^-1 Pa^-1 related)
# α1 from Ciddor (similar context) is Pa-1 C-1. Let's assume t_c units for alpha1, alpha2
# For consistency, let's use the structure from Ciddor's paper for Z factor which is similar
...
# alpha1 is in Pa-1, alpha2 in K-1 Pa-1. This means alpha1 is not temp dependent, alpha2 is.
...
def _calculate_Z(pressure_pa, temp_c):
    ...
    # The term (alpha0 + alpha1*t + alpha2*t^2) appears to mix units if alpha1 is just Pa-1
    ...
    # TODO: Revisit Birch & Downs specific Z constants if high precision difference needed.
...
def _saturation_vapor_pressure_bd(temperature_c):
    ...
    A_svp_c = 1.2378847e-5; B_svp_c = -1.9121316e-2; C_svp_c = 33.93711047; D_svp_c = -6.3431645e3
...
def _partial_pressure_water_vapor_bd(temperature_c, relative_humidity, pressure_pa):
    ...
    f_w = 1.00062 + (3.14e-8 * pressure_pa) + (5.6e-7 * temperature_c**2) # Ciddor's f_w
...
def birch_downs_refractive_index(wavelength_um, conditions):
    ...
    n_s0_minus_1_e8 = BD_A_DISP + BD_B_DISP / (BD_C_DISP - sigma2) + \
                      BD_D_DISP / (BD_E_DISP - sigma2)
    ...
    n_ap_minus_1 = n_as_minus_1 * \
                   (p_pa_actual / P_STD) * (T_STD_KELVIN / t_k_actual) * \
    ...
    if pv_pa == 0:
        ...
        g_t_wv = (1.0 / t_k_actual) * \
                 (1 + ALPHA_E_WV * (t_c_actual - T0_WV_CELSIUS))
        spectral_term_wv = A_WV + B_WV * sigma2 + \
                           C_WV * sigma2**2 + D_WV * sigma2**3
