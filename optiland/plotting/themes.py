...
THEMES = {
    'light': {
        ...
        'legend.labelcolor': '#333333', # Matplotlib uses 'text.color' for legend text by default
...
def get_theme_value(key: str, theme_name: str = None):
  ...
  """
  current_theme_name = theme_name if theme_name else _active_theme
  ...
  if key not in theme_dict:
    # Consider if a more general ConfigurationError is better if keys might not be theme-specific
    # For now, ThemeNotFoundError implies the key is missing from *this* theme's definition.
