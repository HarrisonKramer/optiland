        ...
        for key_part in keys:
            if isinstance(current_value, dict):
                if key_part not in current_value:
                    raise KeyError(f"Key '{key_part}' not found in '{'.join(keys[:keys.index(key_part)]) if keys.index(key_part) > 0 else self.current_theme}' for key '{element_key}'")
                ...
                try:
                    ...
                    raise ValueError(f"Invalid index '{key_part}' in key '{element_key}'. Index must be an integer.")
                except IndexError:
                    raise IndexError(f"Index {index} out of bounds for list in key '{element_key}'")
            else:
                # This case occurs if a key tries to go deeper into a non-dict/list value
                raise KeyError(f"Cannot access sub-key '{key_part}' because "
...
if __name__ == '__main__':
    ...
    config.set_theme("dark")
    print(f"Default patch edge color (dark): {config.get_style('patch_colors.default.edgecolor')}")

    # Test new specific line colors
    config.set_theme("light")
    print(f"\nSurface line color (light): {config.get_style('specific_line_colors.surface')}")
    config.set_theme("dark")
    print(f"Surface line color (dark): {config.get_style('specific_line_colors.surface')}")

    try:
        print(f"Non-existent specific color: {config.get_style('specific_line_colors.non_existent')}")
    except KeyError as e:
