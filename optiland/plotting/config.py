...
def set_config(key: str, value):
  ...
  if not isinstance(value, type(default_value)):
    ...
    if key == 'figure.figsize' and isinstance(default_value, tuple) and isinstance(value, tuple):
        if not (len(value) == len(default_value) and
                all(isinstance(v, (int, float)) for v in value) and
                all(isinstance(dv, (int, float)) for dv in default_value) ): # Ensure default is also numbers
             raise exceptions.ConfigurationError(
  ...
  if key == 'figure.figsize':
      if not (isinstance(value, tuple) and len(value) == 2 and
              all(isinstance(v, (int, float)) and v > 0 for v in value)):
          raise exceptions.ConfigurationError(
...
def load_config(filepath: str):
  ...
  try:
    ...
    raise exceptions.ConfigurationError(f"Configuration file not found: '{filepath}'")
