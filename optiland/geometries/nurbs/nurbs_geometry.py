...
class NurbsGeometry(BaseGeometry):
    ...
    def __init__(
        ...
        self.is_symmetric = (
            False  # TO BE CHECKED. If I put false I get an error calling draw3D
        )
    ...
    def compute_nurbs_coordinates(P, W, p, q, U, V, u, v):
        ...
        B = be.repeat(
            N_basis_u[be.newaxis], repeats=n_dim + 1, axis=0
        )  # shape (ndim+1, n+1, N)
    ...
    def compute_bspline_coordinates(P, p, q, U, V, u, v):
        ...
        B = be.repeat(
            N_basis_u[be.newaxis], repeats=n_dim, axis=0
        )  # shape (ndim, n+1, N)
    ...
    def _plane_surface(self):
        ...
        u_knots = be.concatenate(
            (be.zeros(u_degree), be.linspace(0, 1, n - u_degree + 2), be.ones(u_degree))
        )
        v_knots = be.concatenate(
            (be.zeros(v_degree), be.linspace(0, 1, m - v_degree + 2), be.ones(v_degree))
        )
