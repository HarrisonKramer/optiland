...
class TestStandardGeometry:
    ...
    def test_distance_a_zero_case(self, set_test_backend):
        cs = CoordinateSystem()
        # Scenario 1: Mixed a_quad values (some zero, some non-zero)
        # Ray 1: k=-1 (parabola), L=M=0, N=1. Should make a_quad = 0.
        # Ray 2: k=0 (sphere), L=M=0, N=1. Should make a_quad = N^2 = 1.

        # For Ray 1 (a_quad == 0):
        # k = -1, L=0, M=0, N=1
        # a_quad = k*N^2 + L^2 + M^2 + N^2 = -1*1^2 + 0+0+1^2 = 0.
        # b_quad = 2*k*N*z + 2*L*x + 2*M*y - 2*N*radius + 2*N*z
        #        = 2*(-1)*1*z_r1 - 2*1*radius_r1 + 2*1*z_r1 = -2*radius_r1
        # c_quad = k*z^2 - 2*radius*z + x^2 + y^2 + z^2
        #        = -z_r1^2 - 2*radius_r1*z_r1 + x_r1^2 + y_r1^2 + z_r1^2 = x_r1^2 + y_r1^2 - 2*radius_r1*z_r1
        # Expected t_r1 = -c_quad / b_quad = -(x_r1^2+y_r1^2-2*radius_r1*z_r1) / (-2*radius_r1)
        #             = (x_r1^2+y_r1^2)/(2*radius_r1) - z_r1

        # Let radius_r1 = 10.0, x_r1=1.0, y_r1=0.0, z_r1=-2.0 (ray starting point)
        # expected_t_r1 = (1^2+0^2)/(2*10) - (-2.0) = 1/20 + 2.0 = 0.05 + 2.0 = 2.05
        # Surface sag at (1,0) for parabola (k=-1, R=10) is x^2/(2R) = 1/(2*10) = 0.05.
        # Ray is at z=-2.0, surface is at z_sag=0.05. Distance is 0.05 - (-2.0) = 2.05. Correct.

        # For Ray 2 (a_quad != 0, sphere k=0):
        # k=0, L=0, M=0, N=1
        # a_quad = 0*1^2 + 0+0+1^2 = 1.
        # b_quad = 2*0*1*z_r2 - 2*1*radius_r2 + 2*1*z_r2 = -2*radius_r2 + 2*z_r2
        # c_quad = 0*z_r2^2 - 2*radius_r2*z_r2 + x_r2^2 + y_r2^2 + z_r2^2 = x_r2^2+y_r2^2+z_r2^2-2*radius_r2*z_r2

        # Let radius_r2 = 20.0 (sphere), x_r2=0.0, y_r2=0.0, z_r2=-5.0
        # a_quad = 1
        # b_quad = -2*20 + 2*(-5) = -40 - 10 = -50
        # c_quad = 0+0+(-5)^2 - 2*20*(-5) = 25 + 200 = 225
        # d = b_quad^2 - 4*a_quad*c_quad = (-50)^2 - 4*1*225 = 2500 - 900 = 1600. sqrt(d)=40.
        # t1 = (-b_quad + sqrt(d))/(2*a_quad) = (50+40)/2 = 45
        # t2 = (-b_quad - sqrt(d))/(2*a_quad) = (50-40)/2 = 5
        # z1 = z_r2 + t1*N = -5 + 45*1 = 40
        # z2 = z_r2 + t2*N = -5 + 5*1 = 0
        # Closest to vertex (z=0) is t2. So expected_t_r2 = 5.0.
        # Sag at (0,0) for sphere is 0. Ray is at z=-5. Distance is 0 - (-5) = 5. Correct.

        # Geometry setup: Need different k and radius for different rays.
        # This means we can't use one StandardGeometry for both.
        # So, we test each case separately or use multiple geometries if the method could handle it.
        # The method `StandardGeometry.distance` is on an instance, so it uses its own k and radius.
        # We need two test calls.

        # Test 1: a_quad == 0 case
        geom_parabola = geometries.StandardGeometry(cs, radius=10.0, conic=-1.0)
        rays_parabola = RealRays(
            x=be.array([1.0]), y=be.array([0.0]), z=be.array([-2.0]), # x_r1, y_r1, z_r1
            L=be.array([0.0]), M=be.array([0.0]), N=be.array([1.0]),
            wavelength=0.55, intensity=be.array([1.0])
        )
        dist_parabola = geom_parabola.distance(rays_parabola)
        assert_allclose(dist_parabola, be.array([2.05]))

        # Test 2: a_quad != 0 case (already covered by existing test_distance, but good for isolating)
        geom_sphere = geometries.StandardGeometry(cs, radius=20.0, conic=0.0)
        rays_sphere = RealRays(
            x=be.array([0.0]), y=be.array([0.0]), z=be.array([-5.0]), # x_r2, y_r2, z_r2
            L=be.array([0.0]), M=be.array([0.0]), N=be.array([1.0]),
            wavelength=0.55, intensity=be.array([1.0])
        )
        dist_sphere = geom_sphere.distance(rays_sphere)
        assert_allclose(dist_sphere, be.array([5.0]))

        # Test 3: Mixed case - one ray a_quad=0, other a_quad!=0, using ONE geometry.
        # For this, L,M,N must be different for the rays if k is the same.
        # Or, k must be such that for some rays a_quad=0 and for others not.
        # Let k=-1 (parabola).
        # Ray 1: L=0, M=0, N=1 => a_quad = 0
        # Ray 2: L=0.5, M=0, N=sqrt(1-0.5^2) approx 0.866.
        #        a_quad = -1*N^2 + L^2 + N^2 = L^2 = 0.5^2 = 0.25 != 0
        geom_mixed = geometries.StandardGeometry(cs, radius=10.0, conic=-1.0) # Parabola

        # Ray 1 data (a_quad=0)
        x_r1, y_r1, z_r1 = 1.0, 0.0, -2.0
        L_r1, M_r1, N_r1 = 0.0, 0.0, 1.0
        expected_t_r1 = 2.05

        # Ray 2 data (a_quad != 0)
        x_r2, y_r2, z_r2 = 0.0, 0.0, -5.0 # Start at -5, ray points somewhat upwards
        L_r2, M_r2, N_r2 = 0.5, 0.0, be.sqrt(1.0 - 0.5**2) # approx 0.866
        # a_quad = k*N_r2^2 + L_r2^2 + M_r2^2 + N_r2^2 = L_r2^2 = 0.25
        # b_quad = 2*k*N_r2*z_r2 + 2*L_r2*x_r2 + 2*M_r2*y_r2 - 2*N_r2*radius + 2*N_r2*z_r2
        #        = 2*(-1)*N_r2*(-5) + 0 + 0 - 2*N_r2*10 + 2*N_r2*(-5)
        #        = 10*N_r2 - 20*N_r2 - 10*N_r2 = -20*N_r2
        # c_quad = k*z_r2^2 - 2*radius*z_r2 + x_r2^2 + y_r2^2 + z_r2^2
        #        = -1*(-5)^2 - 2*10*(-5) + 0 + 0 + (-5)^2 = -25 + 100 + 25 = 100
        # d = b_quad^2 - 4*a_quad*c_quad = (-20*N_r2)^2 - 4*0.25*100
        #        = 400 * N_r2^2 - 100 = 400 * (1-0.5^2) - 100 = 400 * 0.75 - 100 = 300 - 100 = 200. sqrt(d) = sqrt(200) approx 14.142
        # t1 = (-b_quad + sqrt(d))/(2*a_quad) = (20*N_r2 + sqrt(200))/(2*0.25)
        #    = (20*0.866025 + 14.142135) / 0.5 = (17.3205 + 14.142135) / 0.5 = 31.462635 / 0.5 = 62.92527
        # t2 = (-b_quad - sqrt(d))/(2*a_quad) = (20*N_r2 - sqrt(200))/(2*0.25)
        #    = (17.3205 - 14.142135) / 0.5 = 3.178365 / 0.5 = 6.35673
        # z1_intersect = z_r2 + t1*N_r2 = -5 + 62.92527 * 0.866025 = -5 + 54.5 = 49.5
        # z2_intersect = z_r2 + t2*N_r2 = -5 + 6.35673 * 0.866025 = -5 + 5.5 = 0.5
        # Closest to vertex (z=0) is t2. So expected_t_r2 = 6.356730249390305

        rays_mixed = RealRays(
            x=be.array([x_r1, x_r2]), y=be.array([y_r1, y_r2]), z=be.array([z_r1, z_r2]),
            L=be.array([L_r1, L_r2]), M=be.array([M_r1, M_r2]), N=be.array([N_r1, N_r2]),
            wavelength=0.55, intensity=be.array([1.0, 1.0])
        )
        dist_mixed = geom_mixed.distance(rays_mixed)
        assert_allclose(dist_mixed, be.array([expected_t_r1, 6.356730249390305]))
