...
@pytest.mark.parametrize("model_name_actual, model_func_direct", [
...
def test_dispatch_to_correct_model(
    typical_conditions, reference_wavelength_um, model_name_actual, model_func_direct, set_test_backend
):
    ...
    # acknowledging the test of dispatching itself might be incomplete if models can't run.
    try:
        ...
        if 'numpy' in str(e) or 'optiland.backend' in str(e):
            pytest.skip(f"Skipping dispatch test for {model_name_actual} due to backend loading issue: {e}")
        else:
            raise # Reraise other ModuleNotFoundErrors


def test_unsupported_model_name(typical_conditions, reference_wavelength_um, set_test_backend):
    ...
    with pytest.raises(ValueError, match="Unsupported air refractive index model: foobar"):
        refractive_index_air(reference_wavelength_um, typical_conditions, model="foobar")
...
def test_wavelength_validation_passed_to_models(typical_conditions, set_test_backend):
    ...
    try:
        ...
        if 'numpy' in str(e) or 'optiland.backend' in str(e):
            pytest.skip(f"Skipping wavelength validation test for birch_downs due to backend loading issue: {e}")
        else:
            raise
    ...
    try:
        ...
        if 'numpy' in str(e) or 'optiland.backend' in str(e):
            pytest.skip(f"Skipping wavelength validation test for kohlrausch due to backend loading issue: {e}")
        else:
            raise
...
def test_temperature_validation_passed_to_models(reference_wavelength_um, set_test_backend):
    ...
    try:
        with pytest.raises(ValueError, match="non-positive denominator"):
            refractive_index_air(reference_wavelength_um, conditions_bad_temp, model="kohlrausch")
    except ModuleNotFoundError as e:
        if 'numpy' in str(e) or 'optiland.backend' in str(e):
