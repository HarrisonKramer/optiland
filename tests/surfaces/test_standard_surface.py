# tests/surfaces/test_standard_surface.py
"""
Tests for the main Surface class in optiland.surfaces.standard_surface.
"""
import pytest
...
class TestSurface:
    """
    Contains tests for the fundamental Surface class, which represents a single
    optical surface in a system.
    """

    def create_surface(self):
        """
        Helper method to create a standard Surface instance for testing.

        This surface is a reflective plane with a simple coating.
        """
        cs = CoordinateSystem()
        geometry = Plane(cs)
    ...
    def test_trace_paraxial_rays(self, set_test_backend):
        """
        Tests that the trace method correctly handles and returns ParaxialRays.
        """
    ...
    def test_trace_real_rays(self, set_test_backend):
        """
        Tests that the trace method correctly handles and returns RealRays.
        """
        surface = self.create_surface()
    ...
    def test_set_semi_aperture(self, set_test_backend):
        """
        Tests that the semi-aperture of the surface can be set correctly.
        """
        surface = self.create_surface()
        r_max = 10.0
    ...
    def test_reset(self, set_test_backend):
        """
        Tests that the reset method clears all stored ray trace data from
        the surface.
        """
        surface = self.create_surface()
        surface.reset()
    ...
    def test_set_fresnel_coating(self, set_test_backend):
        """
        Tests that a FresnelCoating can be correctly applied to the surface's
        interaction model.
        """
        surface = self.create_surface()
    ...
    def test_is_rotationally_symmetric(self, set_test_backend):
        """
        Tests that the `is_rotationally_symmetric` method correctly identifies
        a symmetric surface.
        """
        surface = self.create_surface()
    ...
    def test_is_rotationally_symmetric_false(self, set_test_backend):
        """
        Tests that `is_rotationally_symmetric` correctly identifies a
        non-symmetric surface due to either geometry or coordinate system
        properties.
        """
        surface = self.create_surface()
        ...
        # Test with symmetric geometry but decentered/tilted coordinate system
        surface.geometry.is_symmetric = True
        surface.geometry.cs.rx = 0
    ...
    def test_to_dict(self, set_test_backend):
        """
        Tests the serialization of a Surface instance to a dictionary.
        """
        surface = self.create_surface()
    ...
    def test_from_dict(self, set_test_backend):
        """
        Tests the deserialization of a Surface instance from a dictionary,
        ensuring all attributes are correctly restored.
        """
        surface = self.create_surface()
        data = surface.to_dict()
    ...
    def test_from_dict_missing_type(self, set_test_backend):
        """
        Tests that deserializing a Surface from a dictionary without the 'type'
