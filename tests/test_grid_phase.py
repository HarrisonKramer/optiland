...
@pytest.fixture
def skip_if_torch(set_test_backend):
    if be.get_backend() == "torch":
        pytest.skip("Spline interpolation implementation differs between backends")
...
def test_grid_phase_profile_get_phase(grid_data, skip_if_torch):
    x, y, phase_grid = grid_data
    profile = GridPhaseProfile(x, y, phase_grid)
...
def test_grid_phase_profile_get_gradient(grid_data, skip_if_torch):
    x, y, phase_grid = grid_data
    profile = GridPhaseProfile(x, y, phase_grid)
    # test at a point
    ...
    assert_allclose(grad_x, be.array([1.0]), atol=1e-1)
    assert_allclose(grad_y, be.array([3.0]), atol=1e-1)


def test_grid_phase_profile_get_paraxial_gradient(grid_data, skip_if_torch):
    x, y, phase_grid = grid_data
    profile = GridPhaseProfile(x, y, phase_grid)
