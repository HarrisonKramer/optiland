...
def zernike_optic():
    ...
    optic.add_surface(surface_type='standard', is_stop=True, radius=10, thickness=20, index=1)
...
def test_default_behavior(zernike_optic, set_test_backend):
    ...
    semi_aperture_val = float(semi_aperture) if hasattr(semi_aperture, 'item') else semi_aperture
    norm_radius_val = float(zernike_surface.geometry.norm_radius) if hasattr(zernike_surface.geometry.norm_radius, 'item') else zernike_surface.geometry.norm_radius
