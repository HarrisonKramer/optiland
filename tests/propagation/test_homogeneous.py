...
@pytest.fixture
def basic_rays():
    """A simple RealRays fixture for testing."""
    return RealRays(x=[0], y=[0], z=[0], L=[0], M=[0], N=[1], intensity=[1.0], wavelength=[0.5])


def test_homogeneous_propagation_position_update(basic_rays):
    ...
    assert be.allclose(basic_rays.x, be.array([0.0]))
    assert be.allclose(basic_rays.y, be.array([0.0]))
    assert be.allclose(basic_rays.z, be.array([10.0]))


def test_homogeneous_propagation_no_attenuation_with_k0(basic_rays):
    ...
    assert be.allclose(basic_rays.i, initial_intensity)


def test_homogeneous_propagation_attenuation_with_k_gt_0(basic_rays):
    ...
    assert be.allclose(basic_rays.i, be.array([expected_intensity]))


def test_homogeneous_propagation_normalizes_rays(basic_rays):
    ...
    assert be.allclose(magnitude, be.array([1.0]))
