...
def test_initialization_default():
    ...
    assert isinstance(optimizer._optimizer_map['generic'],
                      type(OptimizerGeneric))
    assert isinstance(optimizer._optimizer_map['least_squares'],
                      type(LeastSquares))
...
def test_run_optimizer_generic():
    ...
    optimizer.add_operand(operand_type='f2', target=25, weight=1,
...
def test_run_optimizer_least_squares():
    ...
    optimizer.add_variable(optic, 'radius', surface_number=1)
    optimizer.add_operand(operand_type='f2', target=2525, weight=1,
                          input_data={'optic': optic})
    result = optimizer.run()
    assert result is not None
