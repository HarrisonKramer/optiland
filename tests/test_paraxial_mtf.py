...
def test_paraxial_surface_mtf_original_issue():
    ...
    singlet.add_surface(
        index=2, radius=np.inf, thickness=0.5, material=materials.IdealMaterial(n=n)
    )
    singlet.add_surface(index=3, f=0.500, surface_type="paraxial", thickness=0.5)
...
def test_paraxial_surface_collimated_output():
    ...
    lens.add_surface(
        index=1, surface_type="paraxial", f=100, thickness=100, is_stop=True
    )
...
def test_paraxial_edge_case_handling():
    ...
    import optiland.backend as be
    from optiland.paraxial import NEAR_ZERO_THRESHOLD, Paraxial
    ...
    with patch.object(paraxial, "_trace_generic") as mock_trace:
        ...
        # Case 2: u[-1] is just below threshold
        mock_trace.return_value = (
            be.array([[1.0], [1.0]]),
            be.array([[0.1], [NEAR_ZERO_THRESHOLD * 0.5]]),
        )
        f2 = paraxial.f2()
        assert be.isinf(f2), "f2 should be infinite when u[-1] is below threshold"

        # Case 3: u[-1] is just above threshold (normal case)
        mock_trace.return_value = (
            be.array([[1.0], [1.0]]),
            be.array([[0.1], [NEAR_ZERO_THRESHOLD * 2.0]]),
        )
        f2 = paraxial.f2()
        assert be.isfinite(f2), "f2 should be finite when u[-1] is above threshold"
...
def test_mtf_with_infinite_fno():
    ...
    """
    from optiland.mtf.geometric import DEFAULT_CUTOFF_FREQ
    ...
    assert max_freq == DEFAULT_CUTOFF_FREQ, (
        "max_freq should use default for infinite FNO"
    )
    ...
    assert max_freq == DEFAULT_CUTOFF_FREQ, "max_freq should use default for NaN FNO"
    assert be.isfinite(max_freq), "max_freq should be finite"
